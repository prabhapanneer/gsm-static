/**
 * Shared GSM contact / enquiry API (POST /contact).
 * Used by: `contact-server/index.mjs` (standalone) and `scripts/dev.mjs` (Vite on same port).
 */
...
export function getSmtpConfig() {
  ...
  const skipEmail =
    strEnv('FORMS_SKIP_EMAIL') === '1' || String(strEnv('FORMS_SKIP_EMAIL')).toLowerCase() === 'true';
...
export function redirectUrls() {
  const ok = strEnv('CONTACT_SUCCESS_REDIRECT') || 'http://localhost/GSM/index.html?contact=sent#contact';
  const err = strEnv('CONTACT_ERROR_REDIRECT') || 'http://localhost/GSM/index.html?contact=error#contact';
  return { ok, err };
}
...
async function sendEnquiryMail({ name, phone, topic, mode }) {
  ...
  if (!cfg.user || !cfg.pass) {
    throw new Error('SMTP not configured. Set SMTP_USER and SMTP_PASS in contact-server/.env (see .env.example).');
  }
  if (!cfg.mailTo) {
 ...
 * @param {{ rootInfo?: boolean }} [options] — if rootInfo is false, skip GET / so the SPA (Vite) can serve index.html
 */
export function registerGsmContactRoutes(app, options = {}) {
  const { rootInfo = true } = options;
